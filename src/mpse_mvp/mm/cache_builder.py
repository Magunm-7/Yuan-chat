...
def build_mm_cache(
    ...
    Assumption (still true after your dialogue-mode changes):
      - turns_upgraded_path contains ONLY user turns (turn_id refers to user-turn index)
      - sft_jsonl samples carry meta.turn_id which matches the upgraded user turn_id
    """
    os.makedirs(out_dir, exist_ok=True)
    wav, sr = load_wav(wav_path)

    # load upgraded turns: turn_id -> row
    up_rows = _safe_load_jsonl(turns_upgraded_path)
    up = {}
    for r in up_rows:
        try:
            tid = int(r["turn_id"])
    ...
    # load sft samples
    sft = _safe_load_jsonl(sft_jsonl)
    ...
    with open(index_path, "w", encoding="utf-8") as f:
        for s in tqdm(sft, desc="MM cache"):
            meta = s.get("meta", {}) or {}
            if "turn_id" not in meta:
                continue
            try:
                tid = int(meta["turn_id"])
            except Exception:
                continue
            ...
            alpha_dict = r.get("alpha", {}) or {}
            a = float(alpha_dict.get("audio", alpha_dict.get("a", alpha_dict.get("A", 0.5))))
            # BUGFIX: use "video" (not "audio") as the primary key for v
            v = float(alpha_dict.get("video", alpha_dict.get("v", alpha_dict.get("V", 0.5))))
            alpha = np.array([a, v], dtype=np.float32)

            mu = _mu_dict_to_vec(r.get("mu", {}) or {}, idx_names)
